...
export default function AskPage() {
  ...
  async function runAsk(mode: AnswerMode) {
    ...
    try {
      const res = await fetch('/api/ask', {
        ...
        body: JSON.stringify({ question, section, mode }),
      });
  ...
  return (
    <div className="space-y-8">
      ...
      {draft && draft.verdict === 'no_info' && (
        <section className="rounded-lg border border-stone-200 bg-white p-6">
          <div className="flex items-center gap-2">
            <h2 className="text-lg font-medium text-stone-900">
              No direct match in the knowledge base
            </h2>
          ...
          <p className="mt-3 text-sm text-stone-700">
            {draft.draft_answer ||
              'The ingested past responses do not directly address this question.'}
          </p>
          <p className="mt-3 text-sm text-stone-600">
            Would you like to generate an inferred response? The system can
            draw on related material in the knowledge base to produce a
            best-effort answer. Any extrapolated claims will be marked
            <span className="font-medium"> (inferred)</span> and every source
            used will be cited. A reviewer should still validate the response
            before it is sent to a prospect.
          </p>
          <div className="mt-4">
            <button
              onClick={() => runAsk('infer')}
              disabled={inferring}
              className="rounded bg-stone-900 px-4 py-2 text-sm font-medium text-white disabled:opacity-50"
            >
              {inferring ? 'Generating inference…' : 'Generate inferred response'}
            </button>
          </div>
          {candidates !== null && (
            <p className="mt-3 text-xs text-stone-500">
              {candidates} candidate pair{candidates === 1 ? '' : 's'} considered;
              none directly addressed the question.
            </p>
          )}
        </section>
      )}
        ...
        <section className="rounded-lg border border-stone-200 bg-white p-6">
          ...
          {draft.mode === 'infer' && (
            <p className="mt-3 text-xs text-stone-600">
              This response was generated by extrapolating from related
              material in the knowledge base. Claims marked
              <span className="font-medium"> (inferred)</span> are not direct
              quotes from past responses and should be verified before being
              sent.
            </p>
          )}
